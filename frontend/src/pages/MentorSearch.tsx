...
// import md5 from "md5"; // add this import at the top
...
//   // ✅ Fetch all mentors initially OR filter by expertise
//   const fetchMentors = async () => {
//     setLoading(true);
//     setError(null);
...
//         id: m.id || m.mentor_id || Math.random().toString(36).substring(2),
//         users: {
//           name: m.users?.name || "Unknown Mentor",
//           bio: m.users?.bio || "No bio available.",
//           profile_pic_url: m.users?.profile_pic_url || "/default-avatar.png",
//         },
//         expertise: Array.isArray(m.expertise)
...
//           : [],
//         hourly_rate: m.hourly_rate || 0,
//         average_rating: m.average_rating || 0.0,
//       }));
...
//   // ✅ Fetch mentors on load (initial) and when search input changes
//   useEffect(() => {
//     const delayDebounce = setTimeout(() => {
//       fetchMentors();
//     }, 400);
    ...
    try {
      ...
      const normalized = (Array.isArray(data) ? data : []).map((m: any) => ({
        ...
        users: {
          ...
          email: m.users?.email || "",
          profile_pic_url:
            m.users?.profile_pic_url ||
            (m.users?.email
              ? `https://www.gravatar.com/avatar/${md5(
                  m.users.email.trim().toLowerCase()
                )}?d=identicon&size=200`
              : "/default-avatar.png"),
        },
        expertise: Array.isArray(m.expertise)
          ? m.expertise
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {mentors.map((mentor, idx) => (
              ...
              >
                <div className="flex items-center gap-4 mb-4">
                  <img
                    src={mentor.users?.profile_pic_url || "/default-avatar.png"}
                    alt={mentor.users?.name || "Mentor"}
                    className="w-12 h-12 rounded-full object-cover"
                  />
                  <div>
